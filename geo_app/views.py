...
def fetch_my_location(request):
    # get_username =user_Account.objects.get(username=request.user)
    try:
        dict ={
            "latitude":g.lat,
            "longitude":g.lng,
        ...
        # if get_username:
        #     query_update=Geo_info.objects.filter(user=get_username).update(user_longitude=g.lng,user_latitude=g.lng
